...
bool w5500_init(std::shared_ptr<LedIndicator> ledIndicator)
{
    ...
    ESP_LOGW(TAG, "W5500 Ethernet driver is not enabled in sdkconfig (CONFIG_ETH_SPI_ETHERNET_W5500=0)");
    return false;
#endif
    ...
    ESP_ERROR_CHECK(esp_event_handler_register(ETH_EVENT, ESP_EVENT_ANY_ID, eth_event_handler, ledIndicator.get()));
    ESP_ERROR_CHECK(esp_event_handler_register(IP_EVENT, IP_EVENT_ETH_GOT_IP, got_ip_event_handler, ledIndicator.get()));

    s_ip_semaphore = xSemaphoreCreateBinary();
    ESP_ERROR_CHECK(esp_eth_start(s_eth_handle));
    ESP_LOGI(TAG, "Ethernet started, waiting for IP...");

    bool got_ip = s_ip_semaphore && xSemaphoreTake(s_ip_semaphore, pdMS_TO_TICKS(10000)) == pdTRUE;
...
{
    return s_got_ip && s_eth_netif != nullptr;
}
