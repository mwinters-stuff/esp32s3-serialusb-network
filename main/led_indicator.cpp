...
LedIndicator::LedIndicator()
    : currentState(LedState::IDLE), strip_handle(NULL) {
...
void LedIndicator::setState(LedState newState) {
  ...
  // The UPLOADING state is high-priority and should only be superseded by an
  // ERROR.
...
void LedIndicator::pulse(uint32_t r, uint32_t g, uint32_t b) {
  ...
  setColor((uint32_t)(r * brightness), (uint32_t)(g * brightness),
...
void LedIndicator::run() {
  while (1) {
    ...
    case LedState::WIFI_DISCONNECTED:
      pulse(255, 165, 0);
      break; // Pulsing Orange
    case LedState::USB_CONNECTED:
      setColor(0, 255, 0);
      break; // Solid Green
    case LedState::WEB_TERMINAL_ACTIVE:
      setColor(0, 255, 255);
      break; // Solid Cyan
    case LedState::UPLOADING:
      pulse(255, 0, 255);
      break; // Pulsing Magenta
    case LedState::ERROR:
      setColor(255, 0, 0);
      break; // Solid Red
    }
