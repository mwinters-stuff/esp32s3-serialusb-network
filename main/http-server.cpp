...
#error                                                                         \
    "WebSocket support is not enabled. Please run 'idf.py menuconfig', go to Component config -> HTTP Server, and enable [ ] Enable Websocket support."
#endif
...
HttpServer::HttpServer(std::shared_ptr<UsbHandler> usbHandler,
                       std::shared_ptr<LedIndicator> led)
    : usbHandler(usbHandler), ledIndicator(led) {
...
void HttpServer::ping_task() {
  // This is a simple keep-alive mechanism to prevent the WebSocket from timing
  // out.
  httpd_ws_frame_t ping_frame = {.final = true,
                                 .fragmented = false,
  ...
  while (true) {
    ...
    if (xSemaphoreTake(ws_clients_mutex, pdMS_TO_TICKS(100)) != pdTRUE) {
      ESP_LOGW(TAG, "Failed to acquire mutex for ping");
      continue;
    }
    ...
    for (auto it = ws_clients.begin(); it != ws_clients.end();) {
      ...
      if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Ping failed for fd %d with error %d, removing client",
                 *it, ret);
...
bool HttpServer::is_authenticated(httpd_req_t *req) {
  ...
  if (httpd_req_get_hdr_value_str(req, "Cookie", cookie_buf,
                                  sizeof(cookie_buf)) == ESP_OK) {
...
esp_err_t HttpServer::firmware_upload_handler(httpd_req_t *req) {
  ...
  if (ledIndicator)
    ledIndicator->setState(LedState::UPLOADING);
  ...
  ESP_LOGI(TAG, "OTA update complete (%d bytes). Rebooting...",
           (int)content_len);
...
void HttpServer::broadcast(const uint8_t *data, size_t len) {
  // No mutex - ping task is disabled to avoid deadlock
  ...
  for (auto it = ws_clients.begin(); it != ws_clients.end();) {
    ...
    // Asynchronous send (no logging here to avoid recursion with WebLogger).
    esp_err_t ret = httpd_ws_send_frame_async(this->server, fd, &ws_pkt);
    if (ret != ESP_OK) {
      // ESP_ERR_NO_MEM (257) means the async send queue is full. This is a
      // transient error. We should not disconnect the client. For this
      // application, dropping the packet is acceptable to avoid blocking the
      // USB handler.
      if (ret == ESP_ERR_NO_MEM) {
        // ESP_LOGW(TAG, "WS async send queue full for fd %d. Dropping packet.",
        //          fd);  // Don't log here to avoid recursion
        ++it; // Move to the next client
      } else {
        // Any other error indicates a client that is no longer valid.
        // Don't log here to avoid recursion with WebLogger
        it = ws_clients.erase(it);
      }
    } else {
      ++it;
...
esp_err_t HttpServer::websocket_handler(httpd_req_t *req) {
  ...
  ESP_LOGI(TAG, "websocket_handler called: fd=%d, method=%d (GET=%d, POST=%d)",
           fd, req->method, HTTP_GET, HTTP_POST);

  if (req->method == HTTP_GET) {
    ESP_LOGI(TAG, "WebSocket handshake from fd %d", fd);

    // Add client to list (no mutex - ping task is disabled)
    if (std::find(ws_clients.begin(), ws_clients.end(), fd) ==
        ws_clients.end()) {
      ws_clients.push_back(fd);
      ESP_LOGI(TAG, "Added client fd %d. Total: %zu", fd, ws_clients.size());

      if (ws_clients.size() == 1 && ledIndicator) {
        ledIndicator->setState(LedState::WEB_TERMINAL_ACTIVE);
      }
    }

    char resp[64];
    snprintf(resp, sizeof(resp), "{\"type\":\"status\", \"connected\": %s}",
             isUSBConnected ? "true" : "false");
    ...
    ESP_LOGI(TAG, "Entering frame loop for fd %d", fd);
  }

  // WebSocket frame processing loop - blocks on httpd_ws_recv_frame
  ...
  while (true) {
    ...
    ESP_LOGD(TAG, "Waiting for frame on fd %d", fd);
    ret = httpd_ws_recv_frame(req, &ws_pkt, 0);
    if (ret != ESP_OK) {
      ESP_LOGI(TAG, "Frame receive failed: %d on fd %d - client disconnected",
               ret, fd);

      auto it = std::find(ws_clients.begin(), ws_clients.end(), fd);
      if (it != ws_clients.end()) {
        ws_clients.erase(it);
        ESP_LOGI(TAG, "Removed client fd %d. Remaining: %zu", fd,
                 ws_clients.size());
      }
      return ret;
    }

    ESP_LOGD(TAG, "Frame: fd=%d, type=%d, len=%zu", fd, ws_pkt.type,
             ws_pkt.len);
    ...
    if (ws_pkt.len > 0) {
      ...
      if (ret != ESP_OK) {
        ESP_LOGW(TAG, "Payload receive failed: %d on fd %d", ret, fd);
        free(buf);
        return ret;
      }

      if (ws_pkt.type == HTTPD_WS_TYPE_TEXT && usbHandler) {
...
esp_err_t HttpServer::fs_upload_handler(httpd_req_t *req) {
  ...
  if (img_size > p->size) {
    ...
    httpd_resp_send(req, "Image exceeds LittleFS partition size",
  ...
  ESP_LOGI(TAG, "Erasing LittleFS partition at 0x%08x, size %u", p->address,
           (unsigned)p->size);
  ...
  while (remaining > 0) {
    ...
    // Bounds guard (shouldn’t be needed due to the remaining check, but belt &
    // braces)
    if (written + r > p->size) {
      free(buf);
      httpd_resp_set_status(req, "413 Payload Too Large");
...
void HttpServer::handle_client_close(int sockfd) {
  ...
  if (it != ws_clients.end()) {
    ws_clients.erase(it);
    ESP_LOGI(TAG, "WS client on fd %d disconnected. Remaining clients: %zu",
             sockfd, ws_clients.size());
...
httpd_handle_t HttpServer::start() {
  ...
  if (httpd_start(&this->server, &config) == ESP_OK) {
    ...
    httpd_uri_t term_uri = {.uri = "/",
                            .method = HTTP_GET,
                            .handler =
                                HTTP_HANDLER(HttpServer, terminal_page_handler),
                            .user_ctx = this,
                            .is_websocket = false,
    ...
    httpd_uri_t upload_uri = {.uri = "/upload.html",
                              .method = HTTP_GET,
                              .handler =
                                  HTTP_HANDLER(HttpServer, upload_page_handler),
                              .user_ctx = this,
                              .is_websocket = false,
                              .handle_ws_control_frames = false,
                              .supported_subprotocol = NULL};
    ...
    httpd_uri_t login_uri = {.uri = "/login.html",
                             .method = HTTP_GET,
                             .handler =
                                 HTTP_HANDLER(HttpServer, login_page_handler),
                             .user_ctx = this,
                             .is_websocket = false,
                             .handle_ws_control_frames = false,
                             .supported_subprotocol = NULL};
    ...
    // DISABLED: Ping task causes mutex deadlock with WebSocket handler
    // xTaskCreate(
    //     [](void *param) { static_cast<HttpServer *>(param)->ping_task(); },
    //     "ws_ping_task", 4096, this, 5, NULL);
  }

  // Set up callbacks for USB events
  if (usbHandler) {
    usbHandler->set_rx_callback([this](const uint8_t *data, size_t len) {
...
void HttpServer::log_to_web(const uint8_t *data, size_t len) {
